...
- `index.html`：静态站根路径入口；`vercel.json` 仅作为历史 Vercel 配置保留。
...
```bash
python3 v1/scripts/package_portable_project.py --no-zip
```

然后把 `dist/shaanxi-capital-market-daily/` 同步到 `gh-pages` 分支并等待线上首页出现当日日期。不会为日常发布生成本地 zip 包。发布地址通常是：
