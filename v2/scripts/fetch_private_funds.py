#!/usr/bin/env python3
"""Fetch the V2 daily AMAC securities-private-fund snapshot.
...
DEFAULT_OUTPUT_DIR = VERSION_DIR / "data" / "daily" / "private" / "reports"
DEFAULT_DATA_DIR = VERSION_DIR / "data" / "daily" / "private"
REPO_ROOT = VERSION_DIR.parent
SOURCE_CONTRACT_PATH = REPO_ROOT / "v2" / "config" / "source-contract.json"


def contract_source_path(key: str) -> Path:
    contract = json.loads(SOURCE_CONTRACT_PATH.read_text(encoding="utf-8"))
    return REPO_ROOT / str(contract[key])


PRIVATE_UNIVERSE_PATH = contract_source_path("privateUniverse")
...
AMAC_POST_ATTEMPTS = 14
AMAC_DETAIL_ATTEMPTS = 10
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "daily复用上一成功年度基线并只核验当日注销；weekly/monthly执行年度全量。"
            "auto在周末或每月1日全量，其余日期增量。"
        ),
    )
    return parser.parse_args()


def load_private_universe(path: Path = PRIVATE_UNIVERSE_PATH) -> dict[str, Any]:
    """Load the reviewed PF1/PF2 contract; weak relationships are never inferred here."""
    payload = json.loads(path.read_text(encoding="utf-8"))
    manual_targets = payload.get("manualTerritorialTargets")
    related_targets = payload.get("relatedTargets")
    if not isinstance(manual_targets, list):
        raise ValueError(f"private-fund universe missing manualTerritorialTargets: {path}")
    if not isinstance(related_targets, list):
        raise ValueError(f"private-fund universe missing relatedTargets: {path}")
    for row in manual_targets:
        if row.get("universeTier") != "PF1" or row.get("relationStrength") != "direct":
            raise ValueError(f"only reviewed direct PF1 targets may enter V2: {row}")
    for row in related_targets:
        if row.get("universeTier") != "PF2" or row.get("relationStrength") not in {"substantive", "institutional"}:
            raise ValueError(f"only reviewed PF2 targets may enter V2: {row}")
    return payload
...
def latest_prior_report(data_dir: Path, report_day: dt.date) -> dict[str, Any] | None:
    candidates: list[tuple[dt.date, Path]] = []
    for path in data_dir.glob("security-private-fund-daily-*.json"):
...
def merge_manager_rows(*groups: list[dict[str, Any]]) -> list[dict[str, Any]]:
    merged: dict[str, dict[str, Any]] = {}
    for rows in groups:
        for row in rows:
            key = str(
                row.get("managerId")
                or row.get("userTenantId")
                or row.get("registerNo")
                or row.get("managerName")
                or row.get("name")
                or row.get("url")
                or ""
            )
...
def amac_ip_for_attempt(attempt: int) -> str | None:
    ...
    if _AMAC_PREFERRED_IP in ips:
        ...
        ordered = ips
    # The system resolver is a separate transport and must be tried as well as
    # pinned CDN addresses.  AMAC occasionally accepts one path while returning
    # an empty response on another.
    transports: list[str | None] = [None, *ordered]
    return transports[attempt % len(transports)]


def refresh_amac_ips() -> None:
    """Discard a possibly stale CDN resolution before the second retry wave."""
    global _AMAC_IPS
    _AMAC_IPS = None
...
def post_json(path: str, payload: dict[str, Any], page: int = 0, referer: str = MANAGER_REFERER) -> dict[str, Any]:
    ...
    for attempt in range(AMAC_POST_ATTEMPTS):
        transport_attempt = attempt
        if attempt == AMAC_POST_ATTEMPTS // 2:
            refresh_amac_ips()
        if attempt >= AMAC_POST_ATTEMPTS // 2:
            transport_attempt = attempt - AMAC_POST_ATTEMPTS // 2
        url = endpoint(path, page)
        last_url = url
        ip = amac_ip_for_attempt(transport_attempt)
        attempted_transports.append(ip or "system-dns")
        try:
    ...
    raise RuntimeError(
        "AMAC request failed after retries: "
        f"{last_url} transports={','.join(attempted_transports)} {last_error}"
    ) from last_error
...
def get_text(url: str) -> str:
    ...
    last_error: Exception | None = None
    for attempt in range(AMAC_DETAIL_ATTEMPTS):
        transport_attempt = attempt
        if attempt == AMAC_DETAIL_ATTEMPTS // 2:
            refresh_amac_ips()
        if attempt >= AMAC_DETAIL_ATTEMPTS // 2:
            transport_attempt = attempt - AMAC_DETAIL_ATTEMPTS // 2
        ip = amac_ip_for_attempt(transport_attempt)
        try:
...
def generate_report(report: dict[str, Any]) -> str:
    ...
    manual_territorial_managers = report.get("manualTerritorialManagers") or []
    related_managers = report.get("relatedManagers") or []
    ...
    related_lines = [
        (
            f"- {item.get('managerName')}（managerId={item.get('managerId')}，登记编号{item.get('registerNo')}）："
            f"AMAC当前注册地/办公地为{item.get('currentRegisterProvince')} / {item.get('currentOfficeProvince')}，不是陕西注册；"
            f"陕西关系/办公地为{item.get('inclusionReason')}，按人工准入PF2陕西强关联对象纳入。"
        )
        for item in related_managers
    ] or ["- 无人工准入PF2陕西强关联对象。"]
    ...
    lines = [
        ...
        "",
        (
            f"当前观察池：{report['observationManagerCount']} 家，其中AMAC当前属地查询{report['shaanxiOfficeManagerCount']}家、"
            f"人工复核PF1补充{len(manual_territorial_managers)}家、PF2重要关联{len(related_managers)}家；"
            f"产品备案统计窗口：{report['shaanxiStartDate']} 至 {end}。"
        ),
        "",
...
def main() -> int:
    ...
    prior_report = latest_prior_report(data_dir, end_date)
    ...
    if scan_mode == "daily" and prior_report is None:
        scan_mode = "weekly"
    start_date = end_date
    ...
    if scan_mode == "daily" and prior_report is not None:
        enriched_year_additions = merge_manager_rows(
            prior_report.get("nationalYearAdditions", []),
            enriched_additions,
        )
        enriched_year_cancelled = merge_manager_rows(
            prior_report.get("nationalYearCancellations", []),
            enriched_cancelled,
        )
        year_cancel_detail_limit_hit = False
        year_cancel_detail_total = len(enriched_year_cancelled)
        year_cancelled_for_detail = cancelled_for_detail
    elif year_start_date == start_date:
    ...
    private_universe = load_private_universe()
    manual_territorial_targets = private_universe["manualTerritorialTargets"]
    related_targets = private_universe["relatedTargets"]
    observation_targets = [*manual_territorial_targets, *related_targets]
    ...
    # Manual PF1/PF2 targets are reviewed explicit allow-lists. This intentionally
    # does not derive candidates from people, alumni or search snippets.
    manager_ids.update(str(item.get("managerId") or "") for item in observation_targets)
    manager_ids.discard("")
    manager_names.update(str(item.get("managerName") or "") for item in observation_targets)
    ...
    shaanxi_query_start = start if scan_mode == "daily" else shaanxi_start
    shaanxi_additions = fetch_manager_additions(shaanxi_query_start, end)
    ...
    shaanxi_cancel_start = start_date if scan_mode == "daily" else shaanxi_start_date
    shaanxi_cancel_candidates = fetch_cancelled_by_keywords(
        shaanxi_cancel_start, end_date, shaanxi_keywords, max_pages=20
    )
    ...
    if scan_mode == "daily" and prior_report is not None:
        enriched_shaanxi_additions = merge_manager_rows(
            prior_report.get("shaanxiAdditions", []),
            enriched_shaanxi_additions,
        )
        enriched_shaanxi_cancelled = merge_manager_rows(
            prior_report.get("shaanxiCancellations", []),
            enriched_shaanxi_cancelled,
        )
    ...
    report = {
        ...
        "observationManagerCount": len({
            *(str(item.get("registerNo") or item.get("managerName") or "") for item in shaanxi_managers),
            *(str(item.get("registerNo") or item.get("managerName") or "") for item in observation_targets),
        } - {""}),
        "shaanxiOfficeProducts": shaanxi_products,
        "manualTerritorialManagers": manual_territorial_targets,
        "relatedManagers": related_targets,
        "privateUniverseSource": str(PRIVATE_UNIVERSE_PATH),
