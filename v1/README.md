...
- 逐公司 CNINFO JSON 生成并抽取 PDF 文本后，应先运行 `scripts/render_vr_workbench.py --date YYYY-MM-DD` 生成 VR 精读选题台 HTML/CSV；该工作台借鉴 RM Dashboard 的证券业务切口，但以 PDF 正文证据、兜底防漏和精读优先级为核心，用于正式早报前选题和复盘，不作为正式稿。
...
```text
运行 V1版陕西资本市场动态总流程，生成今天四个频道，完整检索并精读原文；低频频道如无内容变化，沿用上一期模板只改日期出当日图；上传 ima，并发布 GitHub Pages。发布完成前必须直接检查 https://refrain97.github.io/shaanxi-capital-market-daily/v1/ 返回今天 YYYY-MM-DD 更新。
```

如需指定日期，把“今天”换成具体日期：

```text
运行 V1版陕西资本市场动态总流程，日期 YYYY-MM-DD，生成四个频道，完整检索并精读原文；低频频道如无内容变化，沿用上一期模板只改日期出当日图；上传 ima，并发布 GitHub Pages。发布完成前必须直接检查 https://refrain97.github.io/shaanxi-capital-market-daily/v1/ 返回 YYYY-MM-DD 更新。
```
...
不能只推 `main` 后就回复“已发布”。GitHub Pages 当前读取 `gh-pages` 分支，必须经过 `v1/scripts/publish_v1_to_github_pages.sh` 同步并完成线上日期校验。
...
bash v1/scripts/publish_v1_to_github_pages.sh --date YYYY-MM-DD
```

其中 `publish_v1_to_github_pages.sh` 会自动刷新首页归档、同步 `gh-pages`，并检查 GitHub Pages 线上日期。
...
发布脚本会先运行 `v1/scripts/generate_v1_previews.py` 和 `v1/scripts/update_v1_index.py`，自动扫描四个频道 `outputs/` 并重建历史归档；随后同步 `gh-pages` 并检查线上 GitHub Pages 日期。一般不需要手工维护归档列表。
