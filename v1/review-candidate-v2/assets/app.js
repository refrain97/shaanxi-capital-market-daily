...
const cover=(asOf,eyebrow,title,lead,image,extra="")=>`<section class="cover" style="--image:url('${image}')"><div class="cover-inner"><p class="eyebrow">${esc(eyebrow)}</p><h1>${esc(title)}</h1><p class="lead">${esc(lead)}</p><div class="cover-date">${esc(zhDate(asOf))}</div><div class="actions"><a class="btn" href="#today">阅读本期内容</a><button class="btn ghost copy" type="button">复制链接</button></div>${extra}</div></section>`;
...
function home(data){
  const channels=[
    ["上市公司早报","按公告日期、重要度与公司去重展示当期事项。","上市公司",`/v1/assets/previews/listed-${data.asOf}.webp`,"listed.html"],
    ["证券私募年度库",`${data.private.products.length}只年内新增备案，可按月份、管理人和层级筛选。`,"证券私募",`/v1/assets/previews/private-${data.asOf}.webp`,"private.html"],
    ["收并购年度库",`${data.ma.projects.length}个年度项目，已核验与待补来源分区展示。`,"收并购",`/v1/assets/previews/ma-${data.asOf}.webp`,"ma.html"],
    ["金融招投标","5个正式独立项目及1条待回源观察线索。","金融招投标",`/v1/assets/previews/tender-${data.asOf}.webp`,"tender.html"],
    ["国企动态早报","当日无新增时展示最近一期真实记录。","国企动态","/soe-radar/assets/dashboard_hero.png","soe.html"],
  ];
  const featured=data.homeHighlights.map(item=>card(item.title,item.body,item.category,item.href,item.date)).join("");
  return cover(data.asOf,"SHAANXI CAPITAL MARKET DAILY","陕西资本市场动态","聚合陕西上市公司、证券私募、收并购、金融招投标及国企资本动态。","/soe-radar/assets/dashboard_hero.png")
...
function privatePage(data){
  ...
  const host=document.createElement("div");
  const months=[...Array(7)].map((_,index)=>`2026-${String(7-index).padStart(2,"0")}`);
  const managerCount=new Set(channel.products.map(row=>row.managerName)).size;
  host.innerHTML=cover(data.asOf,"ANNUAL DATABASE · PRIVATE FUND","陕西证券私募年度库","年度备案按月检索；产品统计与管理人观察池分别呈现。",`/v1/assets/previews/private-${data.asOf}.webp`)
    +`<div class="wrap compact-wrap">${section("年度概览",`2026年1月1日至${zhDate(data.asOf)}`,`<div class="private-summary"><div><b>${channel.products.length}只</b><span>年内新增备案产品</span></div><div><b>${managerCount}家</b><span>涉及管理人</span></div><div><b>${channel.managerCounts.total}家</b><span>管理人观察池：PF1 ${channel.managerCounts.PF1} + PF2 ${channel.managerCounts.PF2}</span></div></div><div class="month-strip">${[...Array(7)].map((_,index)=>{const key=`2026-${String(index+1).padStart(2,"0")}`;return `<div><b>${index+1}月</b><span>${channel.annualMonthCounts[key]}只</span></div>`;}).join("")}</div><p class="scan-note">数据更新至${zhDate(data.asOf)}；1—2月未识别新增备案，3月识别1只。</p>`,"today")}<section class="dense-section"><header><h2>年度备案明细</h2><b>全年 ${channel.products.length} 只</b></header><div data-private-products></div></section><section class="dense-section" id="private-pool"><header><h2>证券私募观察池</h2><b>${channel.managerCounts.total} = PF1 ${channel.managerCounts.PF1} + PF2 ${channel.managerCounts.PF2}</b></header><p class="pool-definition">PF1包括陕西注册或外省注册、陕西办公的管理人；PF2包括实质经营、股权关系和协会会员观察。协会会员关系不代表陕西注册或办公。</p><div data-private-pool></div></section><section class="dense-section" id="custodian-ranking"><header><h2>年度产品托管券商统计</h2><b>合计 ${channel.custodianStats.reduce((sum,row)=>sum+row.count,0)} 次</b></header><p class="pool-definition">口径为2026年新增备案产品的公示托管人次数，不等同全部业务合作。</p><div class="custodian-ranking">${channel.custodianStats.map((row,index)=>`<div class="custodian-row${index>=8?" custodian-extra":""}"${index>=8?" hidden":""}><span>${esc(row.label)}</span><i style="--value:${row.count};--max:${channel.custodianStats[0].count}"></i><b>${row.count}</b></div>`).join("")}</div>${channel.custodianStats.length>8?`<button class="archive-toggle" type="button" data-custodian-toggle>展开全部</button>`:""}</section>${section("三个月分享窗口",`${zhDate(data.rules.imageWindow.start)}至${zhDate(data.rules.imageWindow.end)}`,`<div class="compact-status">窗口内备案 ${channel.window.length} 条，详情图 ${Math.ceil(channel.window.length/10)} 张，每张最多10条；网页保留全年记录。</div><p><a class="btn" href="images/index.html">查看分享图</a></p>`)}</div>`;
  ...
  const openMonths=new Set(["2026-07"]);
  const drawProducts=()=>{
...
function maPage(data){
  ...
  const host=document.createElement("div");
  host.innerHTML=cover(data.asOf,"ANNUAL DATABASE · M&A","陕西收并购年度库","已核验项目与待补原始来源项目分区展示；计划节点不作为已发生进展。",`/v1/assets/previews/ma-${data.asOf}.webp`)
    +`<div class="wrap compact-wrap">${section("年度概览",`项目源更新至${zhDate(channel.sourceAsOf)}`,`<div class="dense-kpis"><div><b>${channel.projects.length}个</b><span>年度项目</span></div><div><b>${channel.verifiedProjects.length}个</b><span>已核验原始来源</span></div><div><b>${channel.pendingProjects.length}个</b><span>待补原始来源</span></div><div><b>${channel.window.length}个</b><span>三个月可分享项目</span></div></div>`,"today")}<div data-verified></div><div data-pending></div>${section("口径说明","事实与计划节点分开",`<div class="definition">最近进展日期仅取可访问原始来源确认的公告或实际进展日期。未来付款、股东会和交割安排单列为计划节点，不参与“最新”排序；分享图只纳入已核验项目。</div><p><a class="btn" href="images/index.html">查看分享图</a></p>`)}</div>`;
  ...
  pendingSection.innerHTML=`<div class="section-head"><h2>待补原始来源</h2><p>保留年度线索，不进入首页和分享图</p></div>`;
...
function tender(data){
  ...
  return cover(data.asOf,"DAILY UPDATE · TENDER","陕西金融招投标日报","当前机会、推进项目、已出结果项目与待回源线索分别呈现。",`/v1/assets/previews/tender-${data.asOf}.webp`)
    +`<div class="wrap compact-wrap">${section("年度覆盖概览","同一项目不同阶段不重复计项",`<div class="dense-kpis"><div><b>${channel.projects.length}个</b><span>正式独立项目</span></div><div><b>${channel.pending.length}条</b><span>待回源观察</span></div><div><b>${byGroup("已出结果").length}个</b><span>已出结果</span></div><div><b>5 + 1</b><span>正式项目 + 观察线索</span></div></div>`,"today")}${renderGroup("今日可参与机会",byGroup("今日可参与机会"),`截至${zhDate(data.asOf)}，未识别处于有效报名窗口的新增项目。`)}${renderGroup("正在推进",byGroup("正在推进"),"当前没有仍在推进且尚未出结果的正式项目。")}${renderGroup("已出结果",byGroup("已出结果"),"暂无结果项目。")}${section("待回源观察",`${channel.pending.length}条，不计入5个正式项目`, `<div class="tender-projects">${channel.pending.map(tenderProject).join("")}</div>`)}${section("阅读正式日报",zhDate(data.asOf),`<a class="btn" href="${esc(channel.dailyHref)}">打开当日正式日报</a>`)}</div>`;
}

function soe(data){
  const channel=data.soe;
  const lead=channel.isAsOfFallback
    ?`本期未识别新增，展示最近一期${zhDate(channel.latestRecordDate)}的重点记录。`
    :`展示${zhDate(channel.latestRecordDate)}当期重点记录。`;
  return cover(data.asOf,"DAILY ISSUE · SOE","陕西国企动态早报","资本金融、项目资产、产业经营、风险治理与综合动态并列呈现。","/soe-radar/assets/dashboard_hero.png")
    +`<div class="wrap">${section("最近一期重点",lead,`<div class="records">${channel.latestRecords.map(row=>`<article class="record">${tag(row.category,"gold")}<time>${esc(zhDate(row.publishedAt))}</time><b>${esc(row.title)}</b><p>${esc((row.entities||[]).join("、"))}</p>${source(row.sourceName,row.sourceUrl)}</article>`).join("")}</div>`,"today")}${section("每日早报","保留每日发布入口",`<div class="grid two">${card(`${zhDate(data.asOf)}正式早报`,"查看当日正式日报。","当日",channel.dailyHref)}${card("日期归档","浏览项目内已存在的国企早报。","归档",channel.archiveHref)}</div>`)}${section("历史数据边界","较早期次待继续补充",`<div class="callout">${esc(channel.customerBoundary)}</div>`)}</div>`;
}

function watchlist(data){
  const host=document.createElement("div");
  host.innerHTML=cover(data.asOf,"PUBLIC WATCHLIST","陕西资本市场观察池","分层仅表示陕西关系类型和检索范围，不代表投资评级或重要程度。","/soe-radar/assets/dashboard_hero.png")
    +`<div class="wrap">${section("上市公司观察池",`共${data.listed.entities.length}家`,`<div data-listed></div>`,"today")}${section("证券私募观察池",`共${data.private.managers.length}家`,`<div data-private></div>`)}${section("分层说明","关系类型，不是评级",`<div class="grid">${card("L1","陕西辖区A股"),card("L2","陕西办公或经营的境外上市主体"),card("L3","陕西实质强关联上市公司"),card("PF1 / PF2","属地管理人 / 重要关联管理人")}</div>`)}</div>`;
  $("[data-listed]",host).replaceWith(poolTable(data.listed.entities,{kind:"listed"}));
  $("[data-private]",host).replaceWith(poolTable(data.private.managers,{kind:"private"}));
  return host;
}

async function main(){
  const buildVersion=document.body.dataset.buildVersion;
  const response=await fetch(`data/candidate-data.json?v=${encodeURIComponent(buildVersion)}`,{cache:"no-store"});
  if(!response.ok)throw Error("数据加载失败");
  const data=await response.json();
  if(data.build.version!==buildVersion)throw Error("页面资源版本不一致，请刷新后重试");
  const renderers={index:home,listed,private:privatePage,ma:maPage,tender,soe,watchlist};
