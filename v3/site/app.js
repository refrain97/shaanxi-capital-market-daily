...
function renderDashboard() {
  ...
  const activeSignals = snapshot.activeSignalIds.map(signalById).filter(Boolean).sort((a, b) => priorityRank[a.priority] - priorityRank[b.priority]);
  const newEvents = snapshot.newEventIds.map(eventById).filter(Boolean);
  const updatedEvents = snapshot.updatedEventIds.map(eventById).filter(Boolean);
  const actionEvents = state.data.events.filter((item) => item.eventStatus === "action_window" && item.deadlineAt && new Date(item.deadlineAt) > new Date(`${snapshot.date}T00:00:00+08:00`)).sort((a, b) => new Date(a.deadlineAt) - new Date(b.deadlineAt));
  const riskCount = activeSignals.filter((item) => item.signalStatus === "risk").length;
  const dataCount = activeSignals.filter((item) => item.signalStatus === "data_update").length;
  const reviewCount = state.data.providers.filter((item) => item.status !== "PASS").length + state.data.events.filter((item) => ["pending", "conflict"].includes(item.qualityStatus)).length;
  const slotLabel = state.slot === "morning" ? "上午更新" : "盘后更新";
  const important = activeSignals.slice(0, 6);
  const changes = [
    ["新增", newEvents[0]?.title || "本次无新增"],
    ["进展", updatedEvents[0]?.title || "本次无进展"],
    ["关闭", state.slot === "closing" ? "招投标结果仅作历史回溯" : "无关闭事项"],
    ["回溯", state.slot === "closing" ? "并购与国企样例完成归一" : "无回溯补录"]
  ];
  const channelRows = Object.entries(labels.channels).map(([key, channel]) => {
    const yearCount = state.data.events.filter((item) => item.channel === key).length;
    const currentCount = activeSignals.filter((item) => eventById(item.eventId).channel === key).length;
    return `<button class="channel-row" data-channel-jump="${key}"><span class="channel-icon">${icon(channel.icon)}</span><span><h3>${channel.name}</h3><p>${channel.description}</p></span><span class="channel-count"><strong>${currentCount}</strong><span>本次 / 年内${yearCount}</span></span></button>`;
  }).join("");

  const byMonth = Array.from({ length: 12 }, (_, index) => state.data.events.filter((item) => new Date(item.publishedAt).getMonth() === index).length);
  const maxMonth = Math.max(...byMonth, 1);
  const monthBars = byMonth.map((count, index) => `<div class="month-bar"><strong>${count || ""}</strong><i style="height:${Math.max(4, count / maxMonth * 126)}px"></i><span>${index + 1}月</span></div>`).join("");
  const upcoming = state.watchItems.filter((item) => item.nextReviewAt && !["resolved", "closed"].includes(item.watchStatus)).sort((a, b) => new Date(a.nextReviewAt) - new Date(b.nextReviewAt)).slice(0, 5);

  $("#view-dashboard").innerHTML = `
    ${pageHeading(slotLabel, "今日看板", "先看变化和行动，再进入年度数据与证据。", `样例事件库 ${state.data.events.length} 条<br>质量 ${snapshot.qualityResult}`)}
    <div class="kpi-grid">
      ${kpi("本次新增信号", newEvents.length, "radio-tower", state.slot === "morning" ? "隔夜至08:20" : "相对上午", "signals")}
      ${kpi("当前行动窗口", actionEvents.length, "timer", "按截止日升序", "leads")}
      ${kpi("风险预警", riskCount, "triangle-alert", "已回源事项", "signals")}
      ${kpi("本次数据更新", dataCount + updatedEvents.length, "table-2", "新增与进展分开", "annual")}
      ${kpi("待核验 / 降级", reviewCount, "shield-alert", "AMAC累计受限", "quality")}
    </div>
    <div class="dashboard-primary">
      <section class="panel"><div class="panel-head"><h2>今日最重要</h2><button data-view="signals">查看全部 ${icon("arrow-right")}</button></div><div class="signal-list">${important.length ? important.map(signalRow).join("") : emptyState("本次无重点信号")}</div></section>
      <section class="panel"><div class="panel-head"><h2>行动窗口 / 最近截止</h2><span>${actionEvents.length} 个有效节点</span></div><div class="deadline-list">${actionEvents.length ? actionEvents.map(deadlineRow).join("") : emptyState("当前无行动窗口")}</div></section>
    </div>
    <section class="change-strip"><div class="change-title"><strong>本次变化</strong><span>相对上一成功快照</span></div>${changes.map(([label, value]) => `<div class="change-item"><span>${label}</span><strong>${esc(value)}</strong></div>`).join("")}</section>
    <section class="panel channel-panel"><div class="panel-head"><h2>六频道摘要</h2><span>频道是筛选维度，不重复建事件</span></div><div class="channel-grid">${channelRows}</div></section>
    <div class="dashboard-lower">
      <section class="panel"><div class="panel-head"><h2>2026 年事件节奏</h2><button data-view="annual">进入年度数据 ${icon("arrow-right")}</button></div><div class="trend-wrap"><div class="month-chart">${monthBars}</div></div></section>
      <section class="panel"><div class="panel-head"><h2>我的跟踪 / 最近复核</h2><button data-view="watch">全部 ${upcoming.length} 项 ${icon("arrow-right")}</button></div><div class="calendar-list">${upcoming.length ? upcoming.map(watchCalendarItem).join("") : emptyState("暂无跟踪事项")}</div></section>
    </div>
    <section class="soe-feature"><div><span class="eyebrow">SOE RADAR · 已接入</span><h2>陕西西安国企动态雷达</h2><p>资本金融、项目资产与风险治理动态已进入统一主体和事件视图。本期新增陕西水务发展虚拟电厂项目线索。</p></div><a href="../../soe-radar/index.html" target="_blank" rel="noreferrer">打开原网页 ${icon("external-link")}</a></section>`;
}
...
function openPrivateManager(registerNo) {
  ...
  $("#drawerBody").innerHTML = `<section class="detail-section"><div class="signal-meta"><span class="badge green">活跃分 ${item.activityScore}</span><span>${item.scoreEvidence.totalProductCount}只存量产品</span><span>${item.scoreEvidence.ytdNewProductCount}只年内备案</span></div><p class="detail-summary">${esc(item.teamSummary)}</p></section><section class="detail-section"><h3>当前高管</h3><div class="private-drawer-people">${executives || emptyState("未提取到高管")}</div></section><section class="detail-section"><h3>公开履历</h3><div class="timeline">${histories || emptyState("未提取到履历")}</div></section><section class="detail-section"><h3>证据</h3><div class="source-link-list"><a href="${esc(item.detailUrl)}" target="_blank" rel="noreferrer"><span>01</span><strong>AMAC管理人详情页</strong>${icon("external-link")}</a></div></section>`;
...
function renderQuality() {
  const providerRows = state.data.providers.map((provider) => `<div class="provider-row"><div><strong>${esc(provider.providerId)}</strong><p>${esc(provider.skillRootId)} · ${shortDateTime(provider.checkedAt)} · 影响 ${provider.affectedChannels.map((item) => labels.channels[item]?.name || item).join("、")}</p></div><strong class="${provider.status === "PASS" ? "status-pass" : "status-review"}">${provider.status}</strong></div>`).join("");
...
function updateRunContext() {
  const snapshot = currentSnapshot();
  $("#dataCutoff").textContent = shortDateTime(snapshot.dataCutoffAt);
  $("#lastSuccess").textContent = snapshot.previousSnapshotId.replace("2026-07-", "07-").replace("-morning", " 上午").replace("-closing", " 盘后");
  $("#navSignalCount").textContent = snapshot.activeSignalIds.length;
  $("#navListedCount").textContent = state.data.listedDaily.universeCount;
  $("#navPrivateCount").textContent = state.privateFund?.summary.newProductCount ?? 0;
  $("#navDealsCount").textContent = state.maProjects?.projectCount ?? 0;
  $("#navTenderCount").textContent = state.tenderRuntime?.summary.activeOpportunityCount ?? state.data.tenderMonitor.activeOpportunityEventIds.length;
  updateWatchCount();
  const degradedCount = state.data.providers.filter((item) => item.status !== "PASS").length;
  const sourcePill = $(".source-pill");
  sourcePill.classList.toggle("degraded", degradedCount > 0);
  $("#sourceHealthLabel").textContent = degradedCount ? `${degradedCount}源降级` : "来源可用";
}
...
function exportAnnualCsv() {
  ...
  link.download = `陕西资本市场V3-${state.annualFilters.channel}-2026.csv`;
  link.click();
...
async function init() {
  try {
    ...
    $("#view-dashboard").innerHTML = `<div class="audit-note">${icon("circle-alert")}<div><strong>数据加载失败</strong><br>${esc(error.message)}。请通过本地HTTP服务打开V3页面。</div></div>`;
    refreshIcons();
  }
}
