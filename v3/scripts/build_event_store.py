...
def import_listed_backfill(store: Store, data: dict[str, Any], universe: dict[str, Any]) -> int:
    active_entity_ids = {entity["entityId"] for entity in universe.get("entities", [])}
    if active_entity_ids:
        placeholders = ",".join("?" for _ in active_entity_ids)
        store.db.execute(
            f"UPDATE entities SET status='excluded' WHERE entity_type='listed_company' AND entity_id NOT IN ({placeholders})",
            tuple(sorted(active_entity_ids)),
        )
    ...
    active_sources = [source for source in data.get("sources", []) if source.get("entityId") in active_entity_ids]
    active_source_ids = {source["sourceRecordId"] for source in active_sources}
    for source in active_sources:
        store.source(source["sourceRecordId"], {"sourceType": "official_announcement", "sourceName": "巨潮资讯", **source})
    active_candidates = [item for item in data.get("candidates", []) if item.get("primaryEntityId") in active_entity_ids]
    for item in active_candidates:
        source_ids = [source_id for source_id in item.get("sourceRecordIds", []) if source_id in active_source_ids]
        store.candidate({"candidateId": item["eventCandidateId"], **item}, "listed", source_ids)
    return len(active_candidates)
