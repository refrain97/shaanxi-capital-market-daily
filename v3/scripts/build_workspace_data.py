...
def sync_listed_backfill_coverage(normalized: dict) -> None:
    path = ROOT / "data/backfill/coverage-2026.json"
    if not path.exists():
        return
    payload = json.loads(path.read_text(encoding="utf-8"))
    current = payload.setdefault("channels", {}).get("listed", {})
    payload["channels"]["listed"] = {
        "status": current.get("status", "RAW_COMPLETE_CLASSIFICATION_PENDING_REVIEW"),
        **normalized["coverage"],
...
def build_listed_workspace() -> dict:
    ...
    sync_listed_backfill_coverage(normalized)
    universe = load("data/listed/universe.json")
    active_entity_ids = {item["entityId"] for item in universe["entities"]}
    active_sources = [item for item in normalized["sources"] if item.get("entityId") in active_entity_ids]
    sources = {item["sourceRecordId"]: item for item in active_sources}
    focus = [
        item for item in normalized["candidates"]
        if item.get("primaryEntityId") in active_entity_ids and item.get("businessPriority") == "focus"
    ]
    ...
    for tier in ("L1", "L2", "L3"):
        tier_sources = [item for item in active_sources if item.get("universeTier") == tier]
        tier_stats.append({
            "tier": tier,
            "subjectCount": len({item["entityId"] for item in tier_sources}),
            "announcementCount": len(tier_sources),
        })
    retrieved_subjects = len({item["entityId"] for item in active_sources})
    ...
    if deep_reads:
        active_names = {item["canonicalName"] for item in universe["entities"]}
        deep_reads["items"] = [item for item in deep_reads.get("items", []) if item.get("companyName") in active_names]
        deep_reads["deepReadItemCount"] = len(deep_reads["items"])
        deep_reads["latestItemCount"] = sum(item.get("reportDate") == deep_reads.get("latestReportDate") for item in deep_reads["items"])
        deep_reads["pdfVerifiedItemCount"] = sum(item.get("evidenceLevel") == "PDF正文数字已核验" for item in deep_reads["items"])
        deep_reads["activeItemCount"] = sum(item.get("workspaceStatus") == "active" for item in deep_reads["items"])
        deep_reads["archivedItemCount"] = sum(item.get("workspaceStatus") == "archived" for item in deep_reads["items"])
    return {
        "schemaVersion": "0.1",
        "year": 2026,
        ...
        "universe": {
            ...
            "announcementCount": len(active_sources),
