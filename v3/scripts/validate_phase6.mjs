...
const privateFund = JSON.parse(fs.readFileSync(path.join(root, "data/private-fund/snapshots/latest.json"), "utf8"));
const errors = [];
...
if (annual.metrics.maProjects !== 25 || annual.metrics.privateManagers !== privateFund.summary.managerCount || annual.metrics.privateYtdProducts !== privateFund.summary.ytdProductCount) errors.push("annual channel metrics do not reconcile");
