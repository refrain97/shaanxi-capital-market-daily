...
def main() -> int:
    ...
    products = []
    for item in source.get("shaanxiOfficeProducts", []):
    ...
    output = {
        ...
        "coverage": {"status": "PRODUCTS_COMPLETE_CANCELLATIONS_LIMITED" if limited else "RAW_COMPLETE", "productCount": len(products), "newManagerCount": len(managers), "cancellationCount": len(cancellations), "yearCancellationDetailChecked": source.get("yearCancelDetailChecked"), "yearCancellationDetailTotal": source.get("yearCancelDetailTotalInWindow"), "limitHit": limited},
